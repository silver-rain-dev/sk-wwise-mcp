...
from core.query import build_object_info_query
...
def test_from_type():
    result = build_object_info_query(from_type=["Sound"])
    assert result["from"] == {"ofType": ["Sound"]}


def test_path_takes_priority_over_type():
    result = build_object_info_query(from_path=["\\Events"], from_type=["Sound"])
...
def test_custom_return_fields():
    result = build_object_info_query(from_path=["\\Events"], return_fields=["id", "name"])
    assert result["options"]["return"] == ["id", "name"]


def test_select_transform():
    result = build_object_info_query(from_path=["\\Events"], select_transform="descendants")
